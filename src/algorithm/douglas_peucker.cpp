...
DouglasPeucker::DouglasPeucker(bool useRelativeEpsilon,
                               double absoluteEpsilon,
                               double relativeEpsilon,
                               double eta)
{
    dpParams.useRelativeEpsilon = useRelativeEpsilon;
    dpParams.absoluteEpsilon = absoluteEpsilon;
    dpParams.relativeEpsilon = relativeEpsilon;
    dpParams.eta = eta;
}

// douglas peucker simplify
bool DouglasPeucker::simplify(const ScopedEDResults& edResults, int width, int height)
{
    ...
    sampleInterval_Li = static_cast<int>(std::round(dpParams.eta * (width + height)));
    sampleInterval_Li = sampleInterval_Li > 1 ? sampleInterval_Li : 1;

    // 3. 计算当前实际使用的 epsilon
    if (dpParams.useRelativeEpsilon)
    {
        // 使用比例值 * 图像长边 (保证无论横屏竖屏，容忍度基准一致)
        currentEpsilon = dpParams.relativeEpsilon * std::max(width, height);
    }
    else
    {
        // 直接使用具体的像素容限
        currentEpsilon = dpParams.absoluteEpsilon;
    }

    if(edResults.data.segmentCount <= 0 || edResults.data.segments == nullptr || width <=0 || height <=0)
    {
        LOG_ERROR << "douglas peucker simplify failed: function param error";
        return false;
    }
