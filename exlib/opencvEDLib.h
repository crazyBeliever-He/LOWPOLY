...
extern "C" {
    ...
    struct EDParams {
        ...
        */
        ...
        double MaxDistanceBetweenTwoLines = 6.0f;

        //! Default value is 1.0
        // 直线拟合误差：最小二乘法拟合时的平均偏差限制. 直线拟合的最大距离误差, 控制像素点偏离理想直线的程度.
        double LineFitErrorThreshold = 1.0f;

        //! Default value is 1.3
        // 圆/椭圆拟合最大误差阈值：像素点到拟合圆/椭圆边界的最大允许距离.
        double MaxErrorThreshold = 1.3f;
    };

    // 1. 基础边缘检测接口：返回所有边缘链
    /**
     * @brief 基础边缘检测入口(ColorED)
     ...
     */
    ED_API EDResults RunEdgeDrawingFull(unsigned char* data, int width, int height, int step, bool isColor, EDParams p);
